 ...
 *
 *  Find the maximum XOR in a given array  with respect of second array.
...
class B_Node2{
    B_Node2[] root = new B_Node2[2]; // 2 , i for 0 and another for 1
    public void set(int bit,B_Node2 node){
        root[bit]=node;
    }

    public B_Node2 get(int bit){
        return root[bit];
    }
...
public class Trie_Maxium_XOR_2 {

    private static B_Node2 root = new B_Node2();

    public Trie_Maxium_XOR_2(){
        root = new B_Node2();
    }
...
//        int [] ar= {6,6,0,6,8,5,6};
//        int [] ar2 = {1,7,1,7,8,0,2};
        int [] ar= {1,2};
        int [] ar2 = {1,1};

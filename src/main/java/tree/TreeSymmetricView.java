...
/**
 * @link = https://www.codingninjas.com/studio/problems/identical-trees_799364?topList=striver-sde-sheet-problems
 *
 *
...
public class TreeSymmetricView {


    // complexity == O(N + M) , space - O(n)
    public static boolean treeSymmetricView(Node<Integer> left,Node<Integer> right)
    {
        if(left==null || right==null)return left==right;

        if(left.value != right.value)return false;

        return treeSymmetricView(left.left,right.right)
                &&
                treeSymmetricView(left.right,right.left);
    }
...
//
    ...
    public static boolean isSymmetric(Node<Integer> root) {
        ...
        return root==null || treeSymmetricView(root.left,root.right);
    }

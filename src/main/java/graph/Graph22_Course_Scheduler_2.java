 ...
 * we need to find it there is any cycle , if yes return empty array as it can not be written in topo order
 *
 *
 * @Author saurabh vaish
 * @Date 26-07-2023
 */
public class Graph22_Course_Scheduler_2 {

    public static ArrayList<Integer> findSchedule(int n, ArrayList<ArrayList<Integer>> pair) {
        ...
        if(list.size()==n)return list; // as topological sort and course size same so its possible to complete all course
    ...
    private static ArrayList<ArrayList<Integer>> adjacencyList(ArrayList<ArrayList<Integer>> pair, int n) {
        ...
        for(int i=0;i<pair.size();i++){
            list.get(pair.get(i).get(1)).add(pair.get(i).get(0)); // as need to complete task [0][1] before [0][0]
        }
    ...
    public static void main(String[] args) {
        int mat [][] = {{1,2},{2,3}}; // no as cycle present
        int n=3, m=2;

//        int mat [][] = {{1,0},{2,1},{3,2}}; // yes as no cycle present
//        int n=4;

//        int mat [][] = {{2,1},{3,1},{4,2},{4,3}}; // yes as no cycle present
//        int n=4;
        ...
        System.out.println(findSchedule(n,arrayList));
    }

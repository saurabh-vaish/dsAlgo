...
public class TicTokToeMiniMaxWithAlphaBetaPruning {

    static char player='o' , computer='x' ,nil='_';
    static boolean isGameStarted=false;
    static long totalMoves=0l;
...
//                { nil, nil, nil }};
        ...
        while(turn) {
            ...
            if(index>9||index<1){
                System.out.println("Invalid , try another move");
                continue;
            }
            int row=(index-1)/Board.BOARDSIZE;
            int col=(index-1)%Board.BOARDSIZE;
            ...
            if(board[row][col]!=nil){
                ...
                Board.availableMoves.remove(index); // remove from available
            }
            ...
            if(bestGameMove.isMoveLeft) {
                ...
                int selectedIndex = bestGameMove.nextRow*Board.BOARDSIZE+bestGameMove.nextCol; // calculate index based on selected
                Board.availableMoves.remove(selectedIndex+1); // remove from available
//                System.out.println("optimal next bestMove = [" + bestMove.nextRow + "," + bestMove.nextCol + "]");
            ...
            System.out.println("total no of moves = "+totalMoves);
            printGame(board);
        }
    ...
    private static GameMove findBestMove(char[][] board, GameMove gameMove) {
        ...
        int movesCount=0;
        if(isGameStarted) {
            for (int mo:Board.availableMoves) {
                totalMoves++;
                int i=(mo-1)/Board.BOARDSIZE;
                int j=(mo-1)%Board.BOARDSIZE;
                if (board[i][j] == nil) {
                    board[i][j]=computer;  // making move for computer

                    // evolute best move
                    int best = minimax(board,false,Integer.MIN_VALUE,Integer.MAX_VALUE);
...
//            System.out.println("bext value = "+bestValue);
    ...
    private static int minimax(char[][] board, boolean isMax,int alpha,int beta) {
